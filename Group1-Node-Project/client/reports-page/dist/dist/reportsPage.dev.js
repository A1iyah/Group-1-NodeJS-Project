...
function HandleEmployeeReport(ev) {
  try {
    ...
    var _a = employeeDetails_1.match(/^(.*?)\s-\s(\d+)$/).slice(1),
        name = _a[0],
        idNumber = _a[1];

    if (!employeeDetails_1) throw new Error("no employee selected");
...
managerButton.addEventListener("click", function (e) {
  ...
  reportsBySalary.style.display = "none";
  reportsByEmployee.style.display = "none";
  fetch("/api/admin/get-managers-list").then(function (res) {
    return res.json();
  }).then(function (data) {
    try {
      if (!data) throw new Error("didn't get any data");
      var managers = data.managers.managers;
      var html1 = managers.map(function (manager) {
...
function HandleManagerReport(ev) {
  try {
    ...
    var _a = managerDetails_1.match(/^(.*?)\s-\s(\d+)$/).slice(1),
        name = _a[0],
        idNumber = _a[1];

    if (!managerDetails_1) throw new Error("no employee selected");
