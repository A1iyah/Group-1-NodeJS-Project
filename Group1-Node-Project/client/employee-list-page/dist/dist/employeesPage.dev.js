...
updateUIForUserType(userType); // Add new employee -
...
function handleCreateEmployee(evt) {
  try {
    ...
    if (!name_1) throw new Error("No name");
    if (!email_1) throw new Error("No email");
    if (!password_1) throw new Error("No password");
    if (!idNumber_1) throw new Error("No idNumber");
    if (!phone_1) throw new Error("No phone");
    if (!birthday_1) throw new Error("No birthday");
    if (!salaryPerHour_1) throw new Error("No salary");
    if (!role_1) throw new Error("No role");
...
function handleCreateManager(evt) {
  try {
    ...
    var birthday = evt.target.elements.birthday.value; // const role = e.target.elements.role.value;
    ...
    if (!salaryPerHour) throw new Error("No salary"); // if (!role) throw new Error("No role");
...
var handleGetWorkers = function handleGetWorkers() {
  try {
    var _id = user._id;
...
var renderEmployeeList = function renderEmployeeList(employees) {
  try {
    console.log(employees);
    var htmlStr = employees.map(function (employee) {
...
var renderManagersList = function renderManagersList(managers) {
  try {
    var htmlStr = managers.map(function (manager) {
